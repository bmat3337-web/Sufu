...
export default function RequestPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!title.trim() || !description.trim() || !location.city.trim()) {
      setError("Add a clear title, details and location so providers can respond.");
      return;
    }
    if (!user) { toast("Sign in to post a request"); openAuth(); return; }
    setError(null); setSaving(true);
    const result = await createRequest(user.id, {
      title, description, category, requestType,
      budget: budget ? Number(budget) : undefined,
      currency, city: location.city, suburb: location.suburb, preferredDate: preferredDate || undefined,
    });
    setSaving(false);
    if (result.error) { setError("We couldn't post your request. Please try again."); return; }
    setSubmitted(true);
    toast("Your request is now visible to relevant providers");
  }

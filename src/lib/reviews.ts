...
export interface SufuReview {
  id:string; engagementId:string; reviewerId:string; revieweeId:string;
  rating:number; body:string|null; createdAt:string;
}
function map(r:Record<string,unknown>):SufuReview{return{
  id:String(r.id), engagementId:String(r.engagement_id ?? ""), reviewerId:String(r.reviewer_id ?? r.author_id ?? ""),
  revieweeId:String(r.reviewee_id ?? r.provider_id ?? ""), rating:Number(r.rating),
